...
<table class="black_table">
    ...
    <s:iterator value="dataList" var="device" status="index">
        <tr>
            ...
            <td>${device.mac}</td>
            <td>
                <span class="delete icon_delete" data-mac="${device.mac}" onclick="firm(this)"></span>
            </td>
        </tr>
    </s:iterator>
    </tbody>
...
<script type="text/javascript">
    var sum =${total};
    $(".total").val(sum);
    var ajax_url = "/cloud_blackList_delete4Mac_null";
    var json_url = "/cloud_blackList_add_null?mac=";
    $(function () {
        sPage(${total});//信息条数
        input_show();
    });
    function this_page(n) {
        ...
        $.getJSON('/cloud_blackList_list_null?page=' + n + '&rows=' + rows + '&total=' + sum,
                function (data) {
        ...
        $(".search").keydown(function (e) {
            ...
            if (e.keyCode == '13') {
                if (_mac != "") {
                    var _tBody = $('.black_table tbody');
                    _tBody.html('')
                    $.getJSON('/cloud_blackList_ajaxLikeList_null?mac=' + _mac,
                            function (data) {
                                $.each(data, function (i, item) {
                                    if (i == 0)_tBody.html('');
                                    _tBody.append('<tr><td>' +
